...
def connect_to_db():
    ...
    "SERVER=db14234.public.databaseasp.net,1433;"
    "DATABASE=db14234;"
    "UID=db14234;"
    "PWD=qA@29aF-X3#j;"
    "TrustServerCertificate=yes;"
...
def insert_data_to_db(conn, title, price, square, rooms, address, district, city, PostedDate, image_urls):
    try:
        # Lấy districtId từ bảng District
        cursor = conn.cursor()
        cursor.execute("""
            SELECT id FROM Districts WHERE [name] = ? AND ProvinceId = (
        ...
        # Chèn dữ liệu vào bảng Properties
        sql_command = """
            INSERT INTO Properties (title, price, square, rooms, address, districtId, PostedDate)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        """
        cursor.execute(
            sql_command,
            (title, price, square, rooms, address, districtId, PostedDate)
        )
        conn.commit()
        print("Dữ liệu đã được chèn vào bảng Properties.")

        # Lấy PropertyId của bản ghi vừa chèn vào
        cursor.execute("SELECT id FROM Properties WHERE title = ?", (title,))
...
def getDataDetail(detail_url):
    ...
    if len(rows) >= 6:
        ...
        if date_cell:
            ...
            print("Không tìm thấy PostedDate")
    else:
        print("Không tìm thấy PostedDate")
...
def crawl_data_info(conn, card, detail_url):
    try:
        ...
        PostedDate, address, district, city, image_urls = getDataDetail(detail_url)
        
        list_city = ["Hà Nội", "Đà Nẵng", "Hồ Chí Minh"]
        if city in list_city:
            # Chèn dữ liệu vào cơ sở dữ liệu
            insert_data_to_db(conn, title, price, square, rooms, address, district, city, PostedDate, image_urls)
